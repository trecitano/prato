...
module FilteringCustomHelpers
  ...
  def result_for(table, scope, filters)
    table.to_table(scope, params: Prato::Query::Parameters.new(filters: filters))
  end
end
...
class TestFilteringCustomDirectColumn < Minitest::Test
  ...
  def setup
    @table = Prato.table(User) do
      column(:name)
      column(:age, filter: ->(scope, operator, value) {
        case operator
        when :eq
...
class TestFilteringCustomAssociationColumn < Minitest::Test
  ...
  def setup
    @table = Prato.table(User) do
      column(:name)
      column(company_name: %i[company name], filter: ->(scope, operator, value) {
        case operator
        when :eq
  ...
  def test_custom_filter_narrows_association_eq
    result = result_for(@table, User.order(:id), filter(:company_name, :eq, "Acme Corp"))
    assert_names(result, %w[Alice Bob])
  end

  def test_custom_filter_eq_excludes_non_tech_companies
    result = result_for(@table, User.order(:id), filter(:company_name, :eq, "Globex"))
    assert_names(result, [])
  end

  def test_custom_filter_falls_back_for_contains
    result = result_for(@table, User.order(:id), filter(:company_name, :contains, "Acme"))
    assert_names(result, %w[Alice Bob])
  end
end
...
class TestFilteringCustomExpressionColumn < Minitest::Test
  ...
  def setup
    @table = Prato.table(User) do
      column(:name)
      column(:age_doubled, expression: "users.age * 2", filter: ->(scope, operator, value) {
        case operator
        when :gte
  ...
  def test_custom_filter_adds_extra_condition_on_gte
    result = result_for(@table, User.order(:id), filter(:age_doubled, :gte, 50))
    assert_names(result, %w[Alice Dave])
  end

  def test_custom_filter_falls_back_for_eq
    result = result_for(@table, User.order(:id), filter(:age_doubled, :eq, 60))
    assert_names(result, ["Alice"])
  end
end
...
class TestFilteringCustomAggregateColumn < Minitest::Test
  ...
  def setup
    @table = Prato.table(User) do
      column(:name)
      column(:post_count, count: :posts, filter: ->(scope, operator, value) {
        case operator
        when :eq
  ...
  def test_custom_filter_counts_only_published_posts
    result = result_for(@table, User.order(:id), filter(:post_count, :eq, 3))
    assert_names(result, ["Alice"])
  end

  def test_custom_filter_falls_back_for_gte
    result = result_for(@table, User.order(:id), filter(:post_count, :gte, 3))
    assert_names(result, %w[Alice Carol])
  end
end
...
class TestFilteringCustomRubyColumn < Minitest::Test
  ...
  def setup
    @table = Prato.table(User) do
      column(:name)
      ruby_column(:name_length, key: :id, filter: ->(actual, operator, value) {
        case operator
        when :eq
  ...
  def test_custom_ruby_filter_overrides_eq_with_fuzzy_match
    result = result_for(@table, User.order(:id), filter(:name_length, :eq, 4))
    assert_names(result, %w[Alice Bob Carol Dave])
  end

  def test_custom_ruby_filter_falls_back_for_gt
    result = result_for(@table, User.order(:id), filter(:name_length, :gt, 4))
    assert_names(result, ["Alice", "Carol"])
  end
end
...
class TestFilteringCustomAndComposition < Minitest::Test
  ...
  def test_custom_filter_chains_with_normal_filter_in_and
    filters = Prato::Query::AndFilter.new([
      filter(:age, :gte, 25),
      filter(:company_name, :eq, "Acme Corp")
    ])
...
class TestFilteringCustomMixedComposition < Minitest::Test
  ...
  def setup
    @table = Prato.table(Comment) do
      column(:body)
      column(:score, filter: ->(scope, operator, value) {
        case operator
        when :gte
  ...
  def test_custom_filter_in_nested_and_inside_or
    filters = Prato::Query::OrFilter.new([
      ...
      filter(:post_title, :eq, "Learning Rails")
    ])
...
class TestFilteringCustomSectionColumn < Minitest::Test
  ...
  def test_custom_filter_works_on_sectioned_column
    result = result_for(@table, User.order(:id), filter(%i[profile age], :eq, 25))
    assert_names(result, ["Carol"])
  end

  def test_custom_filter_fallback_works_on_sectioned_column
    result = result_for(@table, User.order(:id), filter(%i[profile age], :gt, 30))
    assert_names(result, ["Dave"])
  end
end
...
class TestFilteringCustomDefaultFallback < Minitest::Test
  ...
  def setup
    @table = Prato.table(User) do
      column(:name, filter: ->(scope, operator, value) {
        nil
      })
  ...
  def test_always_default_filter_works_with_contains
    result = result_for(@table, User.order(:id), filter(:name, :contains, "ob"))
    assert_names(result, ["Bob"])
  end

  def test_always_default_filter_works_with_in
    result = result_for(@table, User.order(:id), filter(:name, :in, %w[Alice Dave]))
    assert_names(result, %w[Alice Dave])
  end
end
